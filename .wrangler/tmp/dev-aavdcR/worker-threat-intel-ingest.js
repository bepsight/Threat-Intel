...
// .wrangler/tmp/bundle-1SKTXW/checked-fetch.js
...
Be = /* @__PURE__ */ new WeakMap(), _ = /* @__PURE__ */ new WeakMap(), B = /* @__PURE__ */ new WeakMap(), A = /* @__PURE__ */ new WeakMap(), J = /* @__PURE__ */ new WeakMap(), Re = /* @__PURE__ */ new WeakSet(), bt = /* @__PURE__ */ __name(async function(e, r, s = 0) {
  ...
  s("x-format", e.format), s("x-typecheck", e.typecheck), s("x-performance-hints", e.performance_hints), s("x-query-timeout-ms", e.query_timeout_ms), s("x-linearized", e.linearized), s("x-max-contention-retries", e.max_contention_retries), s("traceparent", e.traceparent), s("x-query-tags", e.query_tags, (n) => Object.entries(n).map((o) => o.join("=")).join(",")), s("x-last-txn-ts", a(this, A), (n) => n), s("x-driver-env", a(xt, Be));
...
async function sendToLogQueue(env, logEntry) {
  ...
  logQueue.push({
    ...logEntry,
    timestamp: logEntry.timestamp || (/* @__PURE__ */ new Date()).toISOString()
  });
  if (isSendingLogs)
    return;
  isSendingLogs = true;
...
__name(sendToLogQueue, "sendToLogQueue");
...
var worker_threat_intel_ingest_default = {
  async fetch(request, env) {
    ...
    await sendToLogQueue(env, {
      ...
      data: { url: request.url, timestamp: (/* @__PURE__ */ new Date()).toISOString() }
    });
    const fauna = new Le({ secret: env.FAUNA_SECRET });
    ...
    try {
      if (url.pathname === "/fetchnvd") {
        ...
        await sendToLogQueue(env, {
          ...
          data: { duration, timestamp: (/* @__PURE__ */ new Date()).toISOString() }
        });
...
async function fetchNvdData(env, fauna) {
  ...
  const lastModEndDate = (/* @__PURE__ */ new Date()).toISOString();
  ...
  while (hasMoreData) {
    const requestURL = `https://services.nvd.nist.gov/rest/json/cves/2.0/?resultsPerPage=2000&startIndex=${startIndex}&lastModStartDate=${lastModStartDate}&lastModEndDate=${lastModEndDate}`;
    try {
...
__name(fetchNvdData, "fetchNvdData");
...
__name(processVulnerabilityItem, "processVulnerabilityItem");
async function storeVulnerabilitiesInFaunaDB(vulnerabilities, fauna, env) {
  try {
    ...
    for (const vuln of vulnerabilities) {
      try {
        const query_create = ft`Vulnerabilities.create({ data: ${vuln} })`;
...
__name(storeVulnerabilitiesInFaunaDB, "storeVulnerabilitiesInFaunaDB");
...
// .wrangler/tmp/bundle-1SKTXW/middleware-insertion-facade.js
var __INTERNAL_WRANGLER_MIDDLEWARE__ = [
  middleware_ensure_req_body_drained_default,
  middleware_miniflare3_json_error_default
];
var middleware_insertion_facade_default = worker_threat_intel_ingest_default;
...
// .wrangler/tmp/bundle-1SKTXW/middleware-loader.entry.ts
...
//# sourceMappingURL=worker-threat-intel-ingest.js.map
